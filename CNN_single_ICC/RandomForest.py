...
'''
Params: (n_estimators=500, max_depth=None, min_samples_split=5, min_samples_leaf=2, max_features=sqrt) 
-> Cross-Validated Accuracy: 0.7385, Cross-Validated F1: 0.6693, Cross-Validated AUC: 0.8212 | 
Test Accuracy: 0.7184, Test Precision: 0.7500, Test Recall: 0.5474, Test F1: 0.6329, Test AUC: 0.8298
...
# 迭代所有参数组合
for n, depth, min_split, min_leaf, max_feat in product(n_estimators, max_depth, min_samples_split, min_samples_leaf, max_features):
    # 创建随机森林模型
    ...
    # 执行交叉验证以计算指标
    accuracy_scores = cross_val_score(rf_model, X_train, y_train, cv=kf, scoring='accuracy', n_jobs=-1)
    f1_scores = cross_val_score(rf_model, X_train, y_train, cv=kf, scoring='f1', n_jobs=-1)
    auc_scores = cross_val_score(rf_model, X_train, y_train, cv=kf, scoring='roc_auc', n_jobs=-1)
    ...
    y_proba = rf_model.predict_proba(X_test)[:, 1]  # 预测概率
    ...
    # 保存交叉验证和测试结果
    result_line = (f'Params: (n_estimators={n}, max_depth={depth}, min_samples_split={min_split}, '
                   f'min_samples_leaf={min_leaf}, max_features={max_feat}) -> '
    ...
    if average_auc > best_auc:
        ...
        best_params = (n, depth, min_split, min_leaf, max_feat)

# 准备最佳参数和AUC的输出
best_result = (f'Best Parameters: (n_estimators={best_params[0]}, max_depth={best_params[1]}, '
...
result_file_path = os.path.join(result_folder, f"RandomForest_{current_time}.txt")
