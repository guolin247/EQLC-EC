...
for csv_file in os.listdir(data_folder):
    if csv_file.endswith(".csv"):
        ...
        # 获取标签列并将其从数据中移除
        raw_labels = df.iloc[:, 0].values  # 第一列是标签
        data = df.iloc[:, 2:].values       # 第三列开始是谱图数据
        
        # # 确认数据行数是否为1544，列数是否为511
        # assert data.shape == (:, 204), "数据形状不符合预期"
        #进确认列数size是否为204
        assert data.shape[1] == 199, "数据列数不符合预期"
        ...
        # 处理批次信息，可以从文件名中提取
        batch_name = os.path.splitext(csv_file)[0]  # 使用文件名作为批次
        ...
        for idx, spectrum in enumerate(data):
            ...
            all_labels.append((f"{base_name}_spectrum_{idx+1}.npy", numeric_labels[idx], batch_name))
