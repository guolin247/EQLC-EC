...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.csv'):
        ...
        # 收集每个文件的召回率
        recall_data.append(data['Recall'].values)
...
for name in desired_order:
    if name in algorithm_names:
        ...
        ordered_recall_data.append(recall_data[index])
...
# 绘制箱线图并设置每个箱体的颜色
box = ax.boxplot(ordered_recall_data, labels=ordered_algorithm_names, patch_artist=True)
...
# 保存图片
ax.figure.savefig('recall_boxplot.png', dpi=300)

# 计算并打印每种算法的方差和标准差
for algorithm_name, recalls in zip(ordered_algorithm_names, ordered_recall_data):
