...
# CSV 文件夹路径
folder_path = r"D:\work_GuoLin\machine_learning\machinelearning\src\guolin\CNN_TOMATO\test\result"
...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.csv'):
        ...
        # 收集每个文件的精确率
        precision_data.append(data['Precision'].values)
...
for name in desired_order:
    if name in algorithm_names:
        ...
        ordered_prec_data.append(precision_data[index])
...
# 绘制箱线图并设置每个箱体的颜色
box = ax.boxplot(ordered_prec_data, labels=ordered_algorithm_names, patch_artist=True)
...
# 绘制灰色区域
ax.fill_betweenx([lower_bound, upper_bound], 0.5, len(ordered_algorithm_names) + 0.5, color='#8498AB', alpha=0.2)
...
plt.subplots_adjust(left=0.18, right=0.99, top=0.99, bottom=0.25)

# 自定义 x 轴标签颜色
for label in ax.get_xticklabels():
    if label.get_text() == 'EQLC':
...
# 保存图片
ax.figure.savefig('precision_boxplot.png', dpi=300)

# 计算并打印每种算法的方差和标准差
for algorithm_name, precisions in zip(ordered_algorithm_names, ordered_prec_data):
